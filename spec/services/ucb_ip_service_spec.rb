...
describe UcbIpService do
    ...
    describe 'x-forwarded-for' do
      describe 'staging' do
        ...
        it 'returns true for a campus IP' do
          ...
          expect(UcbIpService.ucb_request?(request)).to be(true)
        end
        ...
        it 'returns false for a non-campus IP' do
          ...
          expect(UcbIpService.ucb_request?(request)).to be(false)
        end
  ...
  describe :campus_ip? do
    ...
    describe 'with IP list available' do
      ...
      it 'returns true for a campus IP' do
        expect(service.campus_ip?('128.32.128.127')).to be(true)
      end

      it 'returns true for an AirBears IP' do
        expect(service.campus_ip?('10.142.128.127')).to be(true)
      end

      it 'returns true for a split tunnel IP' do
        expect(service.campus_ip?('10.136.128.127')).to be(true)
      end

      it 'accepts IPAddr objects' do
        ipaddr = IPAddr.new('128.32.128.127')
        expect(service.campus_ip?(ipaddr)).to be(true)
      end

      it 'returns false for a non-campus IP' do
        expect(service.campus_ip?('8.8.8.8')).to be(false)
      end

      it 'returns false for nil' do
        expect(service.campus_ip?(nil)).to be(false)
      end
    end
    ...
    describe 'with IP list unavailable' do
      it 'returns false if IP list 404s' do
        ...
        expect(service.campus_ip?('128.32.128.127')).to be(false)
      end
